/*!
 * QRCode.js - QR code generator in pure JavaScript
 * MIT License - based on qrcodejs by davidshimjs
 */
(function(global){
'use strict';
...
var moduleCount=qrCode.getModuleCount(),lostPoint=0,row,col;
...
for(var i=0,len=QRRSBlock.getRSBlocks(1,nCorrectLevel);i<=40;i++){
var rsBlocks=QRRSBlock.getRSBlocks(i,nCorrectLevel);
var totalDataCount=0;
for(var j=0;j<rsBlocks.length;j++)totalDataCount+=rsBlocks[j].dataCount;
if(totalDataCount>=length){nType=i;break;}
}
return nType;
}
...
clear:function(){this._oDrawing.clear();}
};
...
this._canvas.width=htOption.width;
this._canvas.height=htOption.height;
...
var _this=this,_htOption=this._htOption,_canvas=this._canvas,_ctx=this._ctx;
var nCount=oQRCode.getModuleCount();
var nWidth=_htOption.width/nCount,nHeight=_htOption.height/nCount;
this.clear();
for(var rowI=0;rowI<nCount;rowI++){
for(var colI=0;colI<nCount;colI++){
_ctx.fillStyle=oQRCode.isDark(rowI,colI)?_htOption.colorDark:_htOption.colorLight;
var nLeft=colI*nWidth,nTop=rowI*nHeight;
_ctx.fillRect(nLeft,nTop,nWidth,nHeight);
}}
this._bIsPainted=true;
},
clear:function(){
this._ctx.clearRect(0,0,this._el.clientWidth,this._el.clientHeight);
this._bIsPainted=false;
},
makeImage:function(){
if(this._bIsPainted){
var _el=document.createElement('img');
_el.src=this._canvas.toDataURL('image/png');
this._el.appendChild(_el);
}
}
};
